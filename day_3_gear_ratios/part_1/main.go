...
func (a *Object) isAdjacent(b Object) bool {
	...
	xMin := a.posX - 1
	xMax := a.posX + a.length + 1

	return b.posX >= xMin && (b.posX+b.length) <= xMax
}
...
func main() {
	...
	for _, number := range numbers {
		for _, symbol := range symbols {
			if number.isAdjacent(symbol.Object) {
				partsNumSumAccum += number.value
				break
			}
		}
	}

	fmt.Println(partsNumSumAccum)
